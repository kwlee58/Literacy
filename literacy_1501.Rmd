...
```{r}
options(width=132)
literacy.1501<-t(read.table("literacy1501.txt", header=T, row.names=1, sep=""))
...
```{r}
literacy.ans.2<-scan(file="literacy_ans.txt")
```
...
```{r}
literacy.type.2<-factor(scan(file="literacy_type.txt", what=character()), levels=c("a", "b", "c"), labels=c("Factual", "Inferential", "Critical"))
```
